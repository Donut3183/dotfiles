...
Config.align_boxed_comment = function()
	-- get visual selection bounds
	local start_line = vim.fn.line("'<")
	local end_line = vim.fn.line("'>")

	-- first pass: find longest inner text
	local max_len = 0
	for ln = start_line, end_line do
		local line = vim.fn.getline(ln)
		-- capture text between the two '*'s
		local inner = line:match("^%s*%*%s*(.-)%s*%*$")
		if inner then
			local w = vim.fn.strdisplaywidth(inner)
			if w > max_len then
				max_len = w
			end
		end
	...
	-- choose at least 80 columns, or expand if any line exceeded that
	local width = math.max(80, max_len)

	-- second pass: rewrite each matched line
	for ln = start_line, end_line do
		local line = vim.fn.getline(ln)
		local inner = line:match("^%s*%*%s*(.-)%s*%*$")
		if inner then
			-- pad inner to 'width', re-box it
			local new_line = string.format(" * %-" .. width .. "s *", inner)
			vim.fn.setline(ln, new_line)
		end
	end
end
